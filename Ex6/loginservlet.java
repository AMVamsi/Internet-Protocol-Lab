...
boolean flag=true; String[] userID=getInitParameter("usernames").split(","); 
String[] password=getInitParameter("passwords").split(","); 
String[] cardids=getInitParameter("cardIDs").split(","); int i; for(i=0;i<userID.length;i++) 
{ 
	if(userID[i].equals(usr) && password[i].equals(pwd)&&cardids[i].equals(card)) 
{ 
flag=false; Cookie cookie1=new Cookie("CurrentUser",usr); 
...
value='Let us C'/>Let us C<br>"); out.println("<input type=radio name='book' 
value='Exploring C'/>Exploring C<br>"); out.println("<input type=radio name='book' 
value='Mastering C'/>Mastering C<br>"); out.println("<input type=submit value='purchase'><hr>");
} } if(flag==true){ 
